...
# The only directories the File Manager may ever touch. Never the whole
# server filesystem -- 'project' is the Django project tree itself, 'media'
# is wherever MEDIA_ROOT actually points (on this deployment that's a
# sibling directory outside BASE_DIR), 'static' is the *source* static dir
# (not STATIC_ROOT, which is generated by collectstatic and blocklisted
# below to avoid a confusing duplicate).
ROOTS = {
    'project': BASE_DIR,
...
# Path segments blocked everywhere, checked case-insensitively. Anything
# starting with '.' is blocked categorically regardless of this set (see
# _is_blocked_segment) -- that alone hides .env, .git, .htaccess, .continue,
# .vscode, etc. without needing to enumerate them one by one.
BLOCKED_SEGMENTS = {
    'venv', 'env', '__pycache__', 'db.sqlite3', 'db.sqlite3.bak',
    'staticfiles', 'node_modules',
    # Stale BASE_DIR/media left over from before MEDIA_ROOT moved outside
    # the project tree. Distinct from the 'media' ROOT key above, which
    # points at settings.MEDIA_ROOT (a different absolute path) -- hiding
    # this avoids a confusing duplicate "media" entry under Project Files.
    'media',
}
